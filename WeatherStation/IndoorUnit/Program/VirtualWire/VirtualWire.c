...
void vw_setup(uint16_t speed)
{
	...
	VW_DPORT |= (1 << VW_TX_PIN);	//pinMode(vw_tx_pin, OUTPUT);
	VW_DPORT &= ~(1 << VW_RX_PIN);	//pinMode(vw_rx_pin, INPUT);
	VW_DPORT |= (1 << VW_PTT_PIN);	//pinMode(vw_ptt_pin, OUTPUT);
	if(INVERTED) VW_PORT |= (1 << VW_PTT_PIN);	//digitalWrite(vw_ptt_pin, vw_ptt_inverted);
	else VW_PORT &= ~(1 << VW_PTT_PIN);
}
...
void vw_tx_start()
{
	...
	if(INVERTED) VW_PORT &= ~(1 << VW_PTT_PIN);	//digitalWrite(vw_ptt_pin, true ^ vw_ptt_inverted);
	else VW_PORT |= (1 << VW_PTT_PIN);
...
void vw_tx_stop()
{
	...
	if(INVERTED) VW_PORT |= (1 << VW_PTT_PIN);	//digitalWrite(vw_ptt_pin, false ^ vw_ptt_inverted);
	else VW_PORT &= ~(1 << VW_PTT_PIN);
    VW_PORT &= ~(1 << VW_TX_PIN);	//digitalWrite(vw_tx_pin, false);
...
{
    vw_rx_sample = ((PIND & (1 << PIND4)) >> PIND4);	//digitalRead(vw_rx_pin);
        ...
        if (vw_tx_index >= vw_tx_len)
	{
        ...
        {
			if(vw_tx_buf[vw_tx_index] & (1 << vw_tx_bit++)) VW_PORT |= (1 << VW_TX_PIN);
			else VW_PORT &= ~(1 << VW_TX_PIN);
